...
then
   ...
   # 说明
show_usage="args: [-u , -p , -P , -d, -v] [--user=, --password=, --port=, --data=, --version=]"
...
GETOPT_ARGS=`getopt -o u:p:P:d:v: -al user:,password:,port:,data:,version: -- "$@"`
...
if [[ -z $opt_user || -z $opt_password || -z $opt_port  || -z $opt_data|| -z $opt_version ]]; then
        echo $show_usage
        echo "opt_user: $opt_user , opt_password: $opt_password , opt_port: $opt_port ,opt_data: $opt_data ,opt_version: $opt_version"
        exit 0
fi
...
docker rm -f mysql
# 启动镜像
docker run -d -p "$opt_port":3306 --privileged=true -v "$opt_data/db:/var/lib/mysql" -e MYSQL_ROOT_PASSWORD="$opt_password" --name mysql mysql:5.7 --character-set-server=utf8mb4 --collation-server=utf8mb4_general_ci
# -d                                          表示后台运行
...
# 修改密码并且创建远程访问权限
GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY "$opt_password" WITH GRANT OPTION;
# 更新权限
flush privileges;
FOF
EOF
...
alias mysql='mysql -h 127.0.0.1 -P 3306'
